...
| `0x02` | **FileHeader** | JSON | `{"path": string, "size": int64, "mode": int, "hash": string}`<br>单个文件的元数据。path 使用 `/` 分隔。 |
| `0x03` | **Chunk** | Binary | `[]byte`<br>文件的原始二进制碎片数据。 |
| `0x04` | **Ack** | JSON | `{"status": "ok" / "error", "code": int, "msg": string}`<br>对上一个关键操作的同步回执。 |
| `0x05` | **Mkdir** | JSON | `{"path": string}`<br>指令：在接收端创建指定的文件夹层级。 |
| `0x06` | **Done** | None | 载荷长度为 0。表示当前任务全部逻辑结束。 |
...
2.  **遍历发送:** 发送端利用 `filepath.Walk` 递归扫描：
    *   **若为目录:** 发送 `Type 05 (Mkdir)`，Payload 携带相对路径。
    *   **若为文件:** 按照第 4 章描述的流程发送文件头和数据块。
3.  **串行同步:** 只有当一个文件的最后一个 `Chunk` 收到 `Ack` 后，才开始下一个文件的 `FileHeader` 发送。
4.  **任务结束:** 遍历完成后发送 `Type 06 (Done)`。
